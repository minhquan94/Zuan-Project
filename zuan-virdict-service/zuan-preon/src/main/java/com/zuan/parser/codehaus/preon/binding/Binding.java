 ...
 * {@link org.codehaus.preon.codec.ObjectCodecFactory}. The reason why it is a
 * public interface instead of an internal one is to allow you to plugin in
 * other kinds of Binding. The typical example here is the
 * {@link ConditionalBindingFactory}. This {@link BindingFactory} creates
 * {@link Binding Binding} instances that respect conditions set as annotations
 * on {@link Fields}.
 * </p>
 */
public interface Binding {
   ...
   * @param resolver
   *          The object capable of returning values for references passed in.
   * @param builder
   *          The builder that will be used when - while loading data from the
   *          {@link BitBuffer} - the Binding is (indirectly) required to create
   *          a default instance of a type.
   * @throws DecodingException
   *           If we fail to decode the fields value from the {@link BitBuffer}.
   */
   ...
   *
   * @return An {@link Expression} evaluating to the amount of bits required for
   *         representing the value handled by this binding.
   */
  Expression<Integer, Resolver> getSize();

  /**
   * Returns a unique identifier for this binding. (Guaranteed to be unique for
   * the Codec created.) Note that decorators are expected to leave this id
   * untouched.
   *
  ...
  /**
   * Saves the state of the bound value to the
   * {@link org.codehaus.preon.channel.BitChannel} passed in.
   *
   * @param value
   *          The value to be stored.
   * @param channel
   *          The channel receiving the encoded representation.
   * @param resolver
   *          The resolver, used to resolve variable references.
   * @throws IOException
