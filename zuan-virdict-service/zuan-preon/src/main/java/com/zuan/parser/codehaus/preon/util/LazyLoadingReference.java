...
public class LazyLoadingReference<T> {
   ...
   * producing an instance of {@link T}. The {@link AtomicReference} makes sure
   * the value can be updated atomically. The {@link SoftReference} makes sure
   * the garbage collector will release the reference if it needs more memory.
   * The {@link Future} makes sure that - once the value has been set, all calls
   * to obtain the data will be blocked until it arrives.
   */
  private final AtomicReference<SoftReference<Future<T>>> reference = new AtomicReference<>();
  ...
  public T get() throws InterruptedException, ExecutionException {
    ...
    while (true) {
      ...
      if (softReference == null) {
        ...
        Callable<T> eval = () -> loader.load();

        // (2) Create a FutureTask, that will eventually hold the result
        FutureTask<T> task = new FutureTask<>(eval);
        ...
        if (reference.compareAndSet(null, softReference)) {
